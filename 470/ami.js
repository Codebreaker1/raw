...
function pieChart() {
  ...
  var chartDat1a = [];               // Chart data (labels, values, and angles)
  ...
  function init() {
    ...
    $('#chartDat1a td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
        chartDat1a[currentRow] = [];
        chartDat1a[currentRow]['label'] = $(this).text();
      } else {
       var value = parseFloat($(this).text());
       totalValue += value;
       value = value.toFixed(2);
       chartDat1a[currentRow]['value'] = value;
      }
    ...
    for ( var slice in chartDat1a ) {
      chartDat1a[slice]['startAngle'] = 2 * Math.PI * currentPos;
      chartDat1a[slice]['endAngle'] = 2 * Math.PI * ( currentPos + ( chartDat1a[slice]['value'] / totalValue ) );
      currentPos += chartDat1a[slice]['value'] / totalValue;
    }
  ...
  function handleChartClick ( clickEvent ) {
    ...
    if ( distanceFromCentre <= chartRadius ) {
      ...
      for ( var slice in chartDat1a ) {
        if ( clickAngle >= chartDat1a[slice]['startAngle'] && clickAngle <= chartDat1a[slice]['endAngle'] ) {
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartDat1a td').removeClass('highlight');
    var labelCell = $('#chartDat1a td:eq(' + (slice*2) + ')');
    var valueCell = $('#chartDat1a td:eq(' + (slice*2+1) + ')');
  ...
  function drawSlice ( context, slice ) {
    ...
    var startAngle = chartDat1a[slice]['startAngle']  + chartStartAngle;
    var endAngle = chartDat1a[slice]['endAngle']  + chartStartAngle;
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      context.fillText( chartDat1a[slice]['label'], centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) );
      context.font = pullOutValueFont;
      context.fillText( pullOutValuePrefix + chartDat1a[slice]['value'] + " (" + ( parseInt( chartDat1a[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 20 );
